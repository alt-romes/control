...
echo "Work in progress... please edit me..."
exit 1
...
const colors = fs.readFileSync(/Users/romes/.cache/wal/colors.css)
...
# Go to discord's core location

cd "$HOME/Library/Application Support/discord"
cd "$(find . -name "0.*" -type d | head -n 1)/modules/discord_desktop_core/"
...
asar extract core.asar core

replace_re="mainWindow\.webContents\.on"
injection=$(echo "mainWindow.webContents.on('dom-ready', () => { mainWindow.webContents.executeJavaScript(\`$css\`); });mainWindow.webContents.on" | tr '\n' ' ')

echo $injection

sed -e "s/$replace_re/$injection/" core/app/mainScreen.js
